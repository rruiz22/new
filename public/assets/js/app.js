...
(function () {
	 ...
	 */
	function safeElement(selector, isId = false) {
		const element = isId ? document.getElementById(selector) : document.querySelector(selector);
		return element;
	}

	function safeSetInnerHTML(selector, html, isId = false) {
		const element = safeElement(selector, isId);
		if (element) {
	...
	function initLanguage() {
		// DON'T automatically call setLanguage on page load to prevent infinite reload loop
		...
		function setupLanguageSelector() {
			...
			Array.from(languages).forEach(function (dropdown, index) {
				...
				newDropdown.addEventListener("click", function (event) {
					...
					const selectedLang = newDropdown.getAttribute("data-lang");
		...
		// Start setup with small delay to ensure DOM is ready
		setTimeout(setupLanguageSelector, 50);
	}
	...
	function setLanguage(lang) {
		...
		if (!document.getElementById("header-lang-img")) {
			console.warn('❌ header-lang-img element not found, retrying in 100ms...');
		...
		if (!baseUrl) {
			console.warn('❌ baseUrl not available yet, retrying in 100ms...');
			setTimeout(() => setLanguage(lang), 100);
			return;
		}
		...
		// Map old language codes to new standardized ones
		const langMapping = {
			'sp': 'es',  // Map 'sp' to 'es'
		...
		if (langMapping[lang]) {
			console.log(`🔄 Mapping ${lang} to ${langMapping[lang]}`);
			lang = langMapping[lang];
		}
		
		// Update flag image based on standardized language codes
		const flagMappings = {
			'en': 'us.svg',
			'es': 'spain.svg',
			'pt': 'br.svg',
		...
		const flagFile = flagMappings[lang] || 'us.svg';
		const flagUrl = baseUrl + "assets/images/flags/" + flagFile;
		console.log(`🎌 Updating flag to: ${flagUrl}`);
		document.getElementById("header-lang-img").src = flagUrl;
		...
		// Call backend to update session and reload page
		const languageUrl = baseUrl + "language/" + lang;
		console.log(`📡 Calling backend: ${languageUrl}`);
		...
		.then(data => {
			...
			if (data.success) {
				...
				// Reload page to update all PHP translations
				setTimeout(() => {
					window.location.reload();
				}, 500);
			} else {
				console.error('⚠️ Error changing language:', data.message);
		...
		.catch(error => {
			console.error('❌ Error calling backend language endpoint:', error);
			// Still reload to ensure consistency
			setTimeout(() => {
				window.location.reload();
			}, 500);
	...
	// Multi language setting - Simplified to use only PHP backend
	function getLanguage() {
		// Function removed - now using only PHP lang() system
		// Page reload handles all translations
	}
	...
	function twoColumnMenuGenerate() {
		// Early return if we're not in a layout that requires these elements
		...
		if (defaultValues && (isTwoColumn == "twocolumn" || defaultValues["data-layout"] == "twocolumn")) {
			if (document.querySelector(".navbar-menu")) {
				safeSetInnerHTML(".navbar-menu", navbarMenuHTML);
			}
			var ul = document.createElement("ul");
			ul.innerHTML = '<a href="#" class="logo"><img src="' + (window.baseUrl || '') + 'assets/images/logo-sm.png" alt="" height="22"></a>';
			...
			safeSetInnerHTML("two-column-menu", ul.outerHTML, true);
	...
	function initLeftMenuCollapse() {
		// Early return if required elements don't exist (like on login page)
		...
		if (document.documentElement.getAttribute("data-layout") == "vertical" || document.documentElement.getAttribute("data-layout") == "semibox") {
			safeSetInnerHTML("two-column-menu", "", true);
			if (document.querySelector(".navbar-menu")) {
				safeSetInnerHTML(".navbar-menu", navbarMenuHTML);
			}
	...
	function initTwoColumnActiveMenu() {
		...
		if (currentPath) {
			if (document.body.className == "twocolumn-panel") {
				var twoColumnMenuItem = twoColumnMenu.querySelector('[href="' + currentPath + '"]');
				if (twoColumnMenuItem) {
					twoColumnMenuItem.classList.add("active");
				}
			}
			// navbar-nav
			...
			if (a) {
				...
				if (parentCollapseDiv && parentCollapseDiv.parentElement.closest(".collapse.menu-dropdown")) {
					...
					if (parentCollapseDiv.parentElement.parentElement.parentElement.parentElement.closest(".collapse.menu-dropdown")) {
						...
						var subMenuItem = twoColumnMenu.querySelector('[href="#' + menuIdSub + '"]');
						if (subMenuItem) {
							subMenuItem.classList.add("active");
						}
					}
					var menuId = parentCollapseDiv.parentElement.closest(".collapse.menu-dropdown").getAttribute("id");
					var mainMenuItem = twoColumnMenu.querySelector('[href="#' + menuId + '"]');
					if (mainMenuItem) {
					...
					if (parentCollapseDiv) {
						...
						var elseMenuItem = twoColumnMenu.querySelector('[href="#' + menuId + '"]');
						if (elseMenuItem) {
							elseMenuItem.classList.add("active");
						}
					}
				}
			} else {
	...
	function initActiveMenu() {
		// Early return if navbar-nav doesn't exist (like on login page)
		var navbarNav = document.getElementById("navbar-nav");
		if (!navbarNav) {
			return;
		}
		...
		if (document.getElementsByClassName("dropdown-item-cart")) {
			...
			function updateCartPrice() {
				...
				if (document.getElementById("cart-item-total")) {
					safeSetInnerHTML("cart-item-total", currencySign + subtotal.toFixed(2), true);
				}
			}
			updateCartPrice();
		}
					...
					if (elem.querySelectorAll(".notification-item").length > 0) {
						...
						if (!emptyNotificationElem) {
							...
							<div class="w-25 w-sm-50 pt-3 mx-auto">\
								<img src="' + ((window.baseUrl || '').replace(/\/$/, '') + '/assets/images/svg/bell.svg') + '" class="img-fluid" alt="user-pic">\
							</div>\
			...
			Array.from(document.querySelectorAll(".notification-check input")).forEach(function (element) {
				element.addEventListener("change", function (el) {
					...
					if (el.target.closest(".notification-item").classList.contains("active")) {
						if (notificationActions) {
							notificationActions.style.display = (checkedCount > 0) ? 'block' : 'none';
						}
					} else {
						if (notificationActions) {
							notificationActions.style.display = (checkedCount > 0) ? 'block' : 'none';
					...
					if (selectContent) {
						safeSetInnerHTML("select-content", checkedCount, true);
					}
				});
				...
				if (notificationDropdown) {
					notificationDropdown.addEventListener('hide.bs.dropdown', function (event) {
						...
						var notificationActions = document.getElementById('notification-actions');
						if (notificationActions) {
							notificationActions.style.display = '';
						}
					});
				}
			});
	...
	function addEventListenerOnSmHoverMenu() {
		// Early return if vertical-hover element doesn't exist (like on login page)
		var verticalHoverElement = document.getElementById("vertical-hover");
	...
	// DEBUGGING: Expose functions globally for testing
	window.setLanguage = setLanguage;
	window.initLanguage = initLanguage;
	
	// Commented out to prevent issues - uncomment only for debugging
	/*
	document.addEventListener('DOMContentLoaded', function() {
		console.log('✅ app.js loaded and initialized');
		console.log('🔗 window.baseUrl available:', typeof window.baseUrl !== 'undefined' ? window.baseUrl : 'NOT DEFINED');
		console.log('🌍 setLanguage function available:', typeof window.setLanguage !== 'undefined');
		console.log('🔧 initLanguage function available:', typeof window.initLanguage !== 'undefined');
	});
	*/
