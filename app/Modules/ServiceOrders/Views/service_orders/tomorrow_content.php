        ...
        <div class="card">
            <div class="card-header d-flex align-items-center">
                ...
                <div class="flex-shrink-0">
                    <button id="refreshTomorrowOrders" class="btn btn-secondary btn-sm">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="tomorrow-service-orders-table" class="table table-borderless table-hover align-middle mb-0 service-orders-table dt-responsive">
...
<script>
function initTomorrowServiceOrdersTable() {
    if (typeof $ === 'undefined') {
        setTimeout(initTomorrowServiceOrdersTable, 100);
        return;
    }
    ...
    if ($.fn.DataTable.isDataTable('#tomorrow-service-orders-table')) {
        $('#tomorrow-service-orders-table').DataTable().destroy();
    }
    ...
    window.tomorrowServiceOrdersTable = $('#tomorrow-service-orders-table').DataTable({
        ...
        columns: window.ServiceOrdersColumnHelpers.generateStandardColumns('<?= base_url() ?>', 'tomorrow'),
        order: [[3, 'asc']],
        pageLength: 25,
        responsive: {
            details: {
        ...
        language: {
            ...
            info: "Showing _START_ to _END_ of _TOTAL_ tomorrow's orders",
            infoEmpty: "No orders for tomorrow",
            infoFiltered: "(filtered from _MAX_ total tomorrow's orders)",
            emptyTable: "No service orders scheduled for tomorrow",
            zeroRecords: "No matching tomorrow's orders found",
            processing: "Loading tomorrow's orders..."
        },
        drawCallback: function(settings) {
            ...
            updateTomorrowOrderCount(info.recordsTotal);
        }
    });
    
    $('#refreshTomorrowOrders').on('click', function() {
        if (window.tomorrowServiceOrdersTable) {
            window.tomorrowServiceOrdersTable.ajax.reload();
        }
    });
