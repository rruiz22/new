...
class InternalNoteModel extends Model
{
    ...
    protected $validationRules = [
        'order_type' => 'required|in_list[sales_order,service_order,car_wash,recon_order]',
        'order_id' => 'required|integer',
        'author_id' => 'required|integer|is_not_unique[users.id]',
        'note' => 'required|min_length[3]|max_length[5000]'
    ];
    ...
    /**
     * Get notes for a specific order with author information
     */
    public function getOrderNotes(int $orderId, array $filters = []): array
    {
        $builder = $this->db->table($this->table . ' n')
            ->select('n.id, n.order_id, n.author_id, n.note, n.mentions, n.attachments, n.parent_note_id, n.created_at, n.updated_at, n.deleted_at, u.first_name, u.last_name, u.username, u.avatar')
        ...
        if (!empty($filters['search'])) {
            $builder->like('n.note', $filters['search']);
        }
     ...
     */
    public function getOrderNotesCount(int $orderId, array $filters = []): int
    {
        $builder = $this->db->table($this->table . ' n')
            ->where('n.order_id', $orderId)
            ->where('n.deleted_at IS NULL')
    ...
    {
        return $this->db->table('note_mentions nm')
            ->select('nm.id, nm.note_id, nm.mentioned_user_id, nm.created_at, nm.read_at, n.note, n.order_id, u.first_name, u.last_name, u.username')
            ->join('internal_notes n', 'nm.note_id = n.id')
            ->join('users u', 'n.author_id = u.id', 'left')
    ...
    {
        return $this->db->table('note_mentions')
    ...
    /**
     * Process attachments JSON field
     */
    private function processAttachments(string $attachmentsJson): array
    {
        ...
        foreach ($attachments as &$attachment) {
            if (isset($attachment['filename'])) {
                $attachment['url'] = base_url('internal-notes/download/' . urlencode($attachment['filename']));
            }
        }
    ...
    {
        log_message('info', 'InternalNoteModel::beforeInsert - Called with data: ' . json_encode($data));
        
        if (isset($data['data']['note'])) {
            // Extract mentions from content
            $mentions = $this->extractMentions($data['data']['note']);
            $data['data']['mentions'] = !empty($mentions) ? json_encode($mentions) : null;
            log_message('info', 'InternalNoteModel::beforeInsert - Extracted mentions: ' . json_encode($mentions));
        }
    ...
    {
        log_message('info', 'InternalNoteModel::afterInsert - Called with data: ' . json_encode($data));
        
        if (isset($data['data']['mentions']) && !empty($data['data']['mentions'])) {
            $mentions = json_decode($data['data']['mentions'], true);
            $noteId = $data['id'];
            
            log_message('info', 'InternalNoteModel::afterInsert - Creating mentions for note ID: ' . $noteId . ', mentions: ' . json_encode($mentions));

            foreach ($mentions as $userId) {
                $this->db->table('note_mentions')->insert([
                    'note_id' => $noteId,
    ...
    protected function afterUpdate(array $data): array
    {
        if (isset($data['data']['mentions']) && isset($data['id'])) {
            ...
            $this->db->table('note_mentions')->where('note_id', $data['id'])->delete();
