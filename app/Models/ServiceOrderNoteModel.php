...
class ServiceOrderNoteModel extends Model
{
    protected $table = 'service_order_notes';
    ...
    /**
     * Get notes for a specific service order with author information
     */
    public function getOrderNotes(int $serviceOrderId, array $filters = []): array
    {
        $builder = $this->db->table($this->table . ' n')
            ...
            ->where('n.service_order_id', $serviceOrderId)
     ...
     */
    public function getOrderNotesCount(int $serviceOrderId, array $filters = []): int
    {
        $builder = $this->db->table($this->table . ' n')
            ->where('n.service_order_id', $serviceOrderId)
            ->where('n.deleted_at IS NULL')
    ...
    {
        return $this->db->table('service_order_note_mentions nm')
            ->select('nm.*, n.content, n.service_order_id, u.first_name, u.last_name, u.username, ai.secret as email')
            ->join('service_order_notes n', 'nm.note_id = n.id')
            ->join('users u', 'n.author_id = u.id', 'left')
    ...
    {
        return $this->db->table('service_order_note_mentions')
    ...
    protected function afterInsert(array $data): array
    {
        if (isset($data['data']['mentions']) && !empty($data['data']['mentions'])) {
            ...
            foreach ($mentions as $userId) {
                $this->db->table('service_order_note_mentions')->insert([
                    'note_id' => $noteId,
    ...
    protected function afterUpdate(array $data): array
    {
        if (isset($data['data']['mentions']) && isset($data['id'])) {
            ...
            $this->db->table('service_order_note_mentions')->where('note_id', $data['id'])->delete();
